...
const PayeeAutocomplete: React.FC<PayeeAutocompleteProps> = ({
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    try {
      setLoading(true);
      setError(null);
      const data = await payeesApi.getAutocomplete(query.trim() || undefined);
      setSuggestions(data);
    } catch (err) {
      console.error('Error fetching payee suggestions:', err);
      setError('Failed to load payee suggestions');
  ...
  const handleSelectPayee = async (payeeName: string) => {
    ...
    if (onCategoryAutoFill) {
      try {
        const lastCategory = await payeesApi.getLastCategory(payeeName);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (selectedIndex >= 0 && allSuggestions[selectedIndex]) {
        ...
        // If no selection, just close suggestions
        setShowSuggestions(false);
      }
